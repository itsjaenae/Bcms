<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <meta name="description" content="">
    <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.3.1/jquery.min.js"></script>
    <script src="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/js/bootstrap.min.js"></script>
    <title>Print Ticket - <?= APP_NAME ?></title>
    <style>
        .center {
            background: #D3D3D3;
            text-align: center;
            padding: 30px 50px;
            justify-content: center;
            margin: 20px;
        }

        form {
            margin: 40px
        }

        .con {
            margin: 20px;
            margin-top: 4rem
        }

        @media print {
            body * {
                visibility: hidden;
            }

            #printable,
            #printable * {
                visibility: visible;
            }

            #printable {
                position: absolute;
                left: 0;
                top: 0;
            }
        }
    </style>
</head>

<?php include '../app/includes/header.php' ?>

<body style="background-color: F5F1F0; margin-top: 5rem;">
    <h2 align="center"><b><img src="https://img.icons8.com/ios-filled/50/000000/summary-list.png" /> Booking Summary</b></h2>
    <br>

    <?php
    if (isset($_GET['pid'])) {
        // Get the 'pid' value
        $pid = $_GET['pid'];


        $selectQuery = "SELECT * FROM `bus_booking` WHERE `booked_id` = :pid LIMIT 1";
        $selectStmt = $pdo->prepare($selectQuery);
        $selectStmt->bindParam(':pid', $pid, PDO::PARAM_STR);
        $selectStmt->execute();


        if ($row = $selectStmt->fetch(PDO::FETCH_ASSOC)) {
            if ($row['booked_id'] == $pid) {
                // Data found, proceed with displaying details
                $seatJson = $row['seat_booking'];
                $seatArray = json_decode($seatJson, true);

    ?>


                <div class="ticket" id="printable">
                    <table class="table table-striped" id="font">

                        <tr>
                            <th>Ticket No</th>
                            <td><?php echo $row['booked_id']; ?></td>
                        </tr>

                        <tr>
                            <th>Card Name</th>
                            <td><?php echo $row['card_name'] ?>

                            </td>
                        </tr>

                        <tr>
                            <th>Email</th>
                            <td><?php echo $row['email'] ?></td>
                        </tr>

                        <tr>
                            <th>Seat Booked</th>
                            <td>
                                <?php
                                if (is_array($seatArray)) {
                                    // Access each element of the array
                                    foreach ($seatArray as $seat) {
                                        echo "Seat NO:" . $seat[0] .  "=> " . $seat[1] . '<br>';
                                    }
                                } else {
                                    echo "No seat information available";
                                }
                                ?>
                            </td>
                        </tr>

                        <tr>
                            <th>Journey Type</th>
                            <td><?php echo $row['j_type'] ?></td>
                        </tr>
                        <tr>
                            <th>Bus No</th>
                            <td><?php echo $row['bus_no'] ?></td>
                        </tr>
                        <tr>
                            <th>No Of Passengers</th>
                            <td><?php echo $row['no_of_pass'] ?></td>
                        </tr>
                        <tr>
                            <th>Departure</th>
                            <td><?php echo $row['departure'] ?></td>
                        </tr>
                        <tr>
                            <th>Departure Date</th>
                            <td><?php echo  date("D d, M Y", strtotime($row['date_of_depart'])) ?></td>
                        </tr>
                        <tr>
                            <th>Destination</th>
                            <td><?php echo $row['destination'] ?></td>
                        </tr>
                        <tr>
                            <th>Date Of Return</th>
                            <td><?php echo  date("D d, M Y", strtotime($row['date_of_return']))  ?  date("D d, M Y", strtotime($row['date_of_return']))  : "No date provided" ?></td>
                        </tr>
                        <tr>
                            <th>Amount</th>
                            <td>$&nbsp;&nbsp;<?php echo $row['total_amount'] ?></td>
                        </tr>

                    </table>
                </div>
                <table align="center" style="margin-top: 3rem;">
                    <tr>
                        <td><a href="javascript:void(0);" onclick="printTicket()">
                                <h3><span style="color:#fff;background-color: black; 
    border-color: black; padding: 0.8rem; border-radius: 0.5rem">
                                        Print Ticket
                                    </span></h3>
                            </a></td>
                    </tr>
                </table>
    <?php
            }
        } else {
            echo "No data found for booked_id: $pid";
        }
    }
    ?>
    <script>
        function printTicket() {
            var printContent = document.getElementById("printable").outerHTML;
            var originalContent = document.body.innerHTML;
            document.body.innerHTML = printContent;
            window.print();
            document.body.innerHTML = originalContent;
        }
    </script>
</body>

</html>